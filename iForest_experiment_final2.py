...
def run_iforest_experiment(machine_type=None, test_all_ids=True, specific_id=None):
    ...
    for machine_id in machine_ids:
        ...
        base_save_dir = os.path.join(os.getcwd(), 'experiment_final2', f"{machine_type.upper()}_{machine_id.split('_')[1].upper()}")
        os.makedirs(base_save_dir, exist_ok=True)

        # Loop sobre max_samples e n_estimators (Eixo Y = max_samples e Eixo X = n_estimators)
        for max_samples in max_samples_values:
            for n_estimators in n_estimators_values:
                ...
                csv_filename = os.path.join(base_save_dir, f'{experiment_name}.csv')
