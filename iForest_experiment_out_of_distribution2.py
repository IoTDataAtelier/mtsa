...
class IForestOODExperiment:
    ...
    def __init__(self, machine_type, train_ids, test_id, max_samples_values, n_estimators_values, k=5):
        """
        ...
        Parâmetros:
            ...
            - max_samples_values (list): Lista de valores para max_samples.
        ...
        self.max_samples_values = max_samples_values
        self.n_estimators_values = n_estimators_values
        self.k = k
        self.contamination = 0.01  
        self.max_features = 1.0  
        self.model_name = 'Isolation_Forest'
        self.save_dir = os.path.join(os.getcwd(), f'experiment_ood_{self.model_name}2_only1Id', self.machine_type.upper())
    ...
    def run(self):
        ...
        for max_samples in self.max_samples_values:
            for n_estimators in self.n_estimators_values:
                
                result = []  # Lista para armazenar os resultados de cada fold
                print('---' * 20)
                print(f'Running for max_samples={max_samples}, n_estimators={n_estimators}')
                print(f'KFold\t|AUC\t|')
                ...
                for i, (train_index, val_index) in enumerate(kf.split(X_train)):
                    ...
                    model_iforest = IForest(n_estimators=n_estimators, contamination=self.contamination,
                                            max_samples=max_samples, max_features=self.max_features)
                    ...
                    result.append([self.model_name, i, n_estimators, self.contamination, max_samples, self.max_features, execution_duration, auc])  
                    print(f'K({i}):\t|{auc:0.5f}\t|')

                # Salvar resultados em CSV
                experiment_name = f'{self.machine_type}_OOD_train-{self.train_ids}_test-{self.test_id}_n_estimators-{n_estimators}_max_samples-{max_samples}'
...
if __name__ == '__main__':
    ...
    train_ids = ["id_02"]
    test_id = "id_00"
    max_samples_values = [128, 512]
    n_estimators_values = [2, 10, 40, 70, 100]
    
    ood_experiment = IForestOODExperiment(machine_type, train_ids, test_id, max_samples_values, n_estimators_values)
    ood_experiment.run()