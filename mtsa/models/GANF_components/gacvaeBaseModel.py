...
class GACVAEBaseModel(NetworkLearnerModel, nn.Module):
    ...
    @property
    def name(self):
        return "GACVAEBaseModel " + "+".join([f[0] for f in self.features])
    ...
    def fit(
        ...
        self.__fitCore(
            loss_best, h_A_old, h_tol, dimension, dataloaders, adjacent_matrix, rho
        )
    ...
    def __score_wave_data(self, X):
        dataloaders = self.__create_dataLoader(
            X, window_size=1, isMonoData=self.isMonoData
        )
        result = []
        for dataloader in dataloaders:
            for x in dataloader:
    ...
    def __create_dataLoader(self, X, batch_size=32, window_size=12, isMonoData=True):
        ...
        for x in X:
            x_dataloader = DataLoader(
                GANFData(x, window_size=window_size),
