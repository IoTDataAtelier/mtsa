...
from mtsa.models.gacvae import GACVAE # noqa: E402
...
def run_gacvae_experiment(data_path, experiment_path,netPath,cuda_device,session_name):
    ...
    model_GACVAE.attach_observer(networkLearnerObserver)
    ...
    finalNet = model_GACVAE.get_adjacent_matrix()
    NetworkLearnerModelResultOrchestrator.save_result(finalNet, auc, "final network", netPath,)
    
    initalNet = model_GACVAE.get_initial_adjacent_matrix()
    model_GACVAE.set_adjacent_matrix(initalNet)
    auc = calculate_aucroc(model_GACVAE, X_test, Y_test)
    NetworkLearnerModelResultOrchestrator.save_result(initalNet,auc, "intial network", netPath )
    
    randomNet = model_GACVAE.get_random_adjacent_matrix()
    model_GACVAE.set_adjacent_matrix(randomNet)
    auc = calculate_aucroc(model_GACVAE, X_test, Y_test)
...
with torch.cuda.device(1):
    if __name__ == "__main__":
        ...
        experiment_path=  "./publications/complenet2025/gacvae/exp05112024/net/exp_" + str(i + 1) + ".avro"
        netPath = "./publications/complenet2025/gacvae/exp05112024/metric/metr_"+ str(i + 1) + ".avro"
        session_name = f"exp_{i}"
        
        set_start_method("spawn")
        info("main line")
        print("session: " + session_name)
        p = Process(target=run_gacvae_experiment, args=(path_input, experiment_path, netPath, 1,session_name))
        p.start()
        p.join()
